...
#include "WDGRemoveHourlyPlaytimeMinder.h"
...
{
    _T("Remove Hourly Playtime-Minder"),
    _T("Removes hourly announces keeping track of your time spent in-game and telling you to stop playing after 3 hours."),
    _T("[UI]"),
    _T(""),
    _T("Ai4rei/AN"),
    1,
    0,
    { 0x6161b1e1, 0xea19, 0x11e1, { 0xa7, 0x78, 0x0, 0x22, 0x15, 0x64, 0x8a, 0x98 } },  /* guid */
    _T(""),
};
...
DiffData* WDGPlugin::GeneratePatch(void)
{
    ...
    try
    {
        // MISSION: Find an unsigned integer division through
        // 3600000 followed by JLE in CGameMode::PollNetworkStatus.

        Fd.uMask = WFD_PATTERN|WFD_WILDCARD|WFD_SECTION;
        Fd.chWildCard = '?';
        Fd.lpszSection = ".text";

        // 2009-06-17aSakexe ~ 2010-08-10aRagexeRE (VC6)
        Fd.lpData =
                  "8BC8"         // MOV     ECX,EAX
                  "2BCA"         // SUB     ECX,EDX
                  ...
                  "C1EF 15"      // SHR     EDI,15h
                  "85FF"         // TEST    EDI,EDI
                  "897D C8"      // MOV     [LOCAL.14],EDI
                  "0F8E"         // JLE     ADDR v
                  ;
        if(this->TryMatch(&Fd, &uOffset))
        {
            this->SetByte(uOffset+21, 0x90);  // JLE -> NOP
            this->SetByte(uOffset+22, 0xE9);  // JLE -> JMP
            break;
        }
