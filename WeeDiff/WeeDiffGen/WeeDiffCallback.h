...
public:
	...
	virtual void UpdateListView() { InvalidateRect(GetDlgItem(m_gui->GetMainHandle(), 1000), NULL, FALSE); }
	virtual UINT32 Match(WeeDiffGenPlugin::LPFINDDATA lpFindData) { return (*m_ragexe)->Match(lpFindData); }
	virtual void Matches(WeeDiffGenPlugin::fnCBAddOffset CBAddOffset, WeeDiffGenPlugin::LPFINDDATA lpFindData) { (*m_ragexe)->Matches(CBAddOffset, lpFindData); }
	...
	virtual void GetSection(CCHAR *lpszSectionName, PIMAGE_SECTION_HEADER lpImageSectionHeader) { (*m_ragexe)->GetSection(lpszSectionName, lpImageSectionHeader); }
	virtual void GetDOSHeader(PIMAGE_DOS_HEADER lpImageDosHeader) { (*m_ragexe)->GetDOSHeader(lpImageDosHeader); }
	virtual void GetNTHeaders(PIMAGE_NT_HEADERS lpImageNTHeaders) { (*m_ragexe)->GetNTHeaders(lpImageNTHeaders); }
