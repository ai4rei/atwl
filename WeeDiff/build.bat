@ECHO OFF
REM (c) 2012-2021 Ai4rei/AN
REM
REM This work is licensed under a
REM Creative Commons BY-NC-SA 4.0 International License
...
SET OUTSTD=CON
:QUIET
SET APPBUILDER=CL /nologo /c /W3 /O2x /GF /EHsc /DWIN32 /DNDEBUG /D_WINDOWS /D_UNICODE /DUNICODE /DSTRICT %%i\*.cpp /I"%BUILDERDIR%\." /I"%BUILDERDIR%\Snippets" /I"%BUILDERDIR%\Common"
SET DLLBUILDER=CL /nologo /W3 /O2x /FD /GF /EHsc /DWIN32 /DNDEBUG /D_WINDOWS /D_UNICODE /DUNICODE /DSTRICT /I"%BUILDERDIR%\." /I"%BUILDERDIR%\Common" %%i\*.cpp /link /NOLOGO /MACHINE:IX86 /DLL /SUBSYSTEM:WINDOWS /OPT:REF /OPT:ICF /RELEASE kernel32.lib user32.lib gdi32.lib comdlg32.lib
...
REM Atwl
FOR %%i IN (WDGEnableAsciiInText WDGRemoveHourlyPlaytimeMinder WDGRemoveHourlyGameGrade WDGRemoveQuakeSkillEffect WDGSharedBodyPalettes WDGSkipPacketHeaderObfuscation WDGSharedHeadPalettes WDGEnable64kHairstyles WDGUseSSOLoginPacket WDGExtendedNPCDialog WDGRemoveLoginErrorCode WDGTranslateClient WDGChatColorGM WDGChatColorNormal WDGChatColorSelf WDGChatColorGuild WDGChatColorParty WDGUseCustomFont WDGScreenshotQuality WDGEnableProxySupport WDGEnableCustom3DBones WDGCancelToLoginWindow WDGRemoveIsCheatName WDGRemoveIsGuildCheatName WDGAllowUnknownSlashCommands) DO %DLLBUILDER% /OUT:"%BUILDERDIR%\Release\plugins\WeeDiffGen\%%i.dll"
GOTO WDGBUILT
:WDGINDIVL
