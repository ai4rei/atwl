...
{
    _T("Chat Color Normal"),
    _T("Changes the text color of normal chat messages."),
    _T("[Color]"),
    _T(""),
    _T("Ai4rei/AN"),
    1,
    0,
    { 0x6161b1ed, 0xea19, 0x11e1, { 0xa7, 0x78, 0x0, 0x22, 0x15, 0x64, 0x8a, 0x98 } },  /* guid */
    _T(""),
};
...
INT32 WDGPlugin::Enabled(void)
{
    ...
    {
        this->m_szColor[0] = 0;
        this->m_dgc->DisplayInputBox(this->GetPluginInfo()->lpszDiffName, _T("Input new color for normal chat text:"), this->m_szColor, _ARRAYSIZE(this->m_szColor));

        if(!this->m_szColor[0])
        {// assume default
            this->m_crChatColor = RGB(0xff,0xff,0xff);
...
DiffData* WDGPlugin::GeneratePatch(void)
{
    ...
    try
    {
        // MISSION: Find CGameMode::Zc_Notify_Chat and replace chat
        // color with user-provided color.
        ...
        Fd.lpData = "68 FF FF FF 00";  // PUSH FFFFFFh (white)
        Fd.uStart = uBOffset;
        Fd.uFinish = uBOffset+0x60;
