...
{
    _T("Chat Color Guild"),
    _T("Changes the text color of guild chat messages."),
    _T("[Color]"),
    _T(""),
    _T("Ai4rei/AN"),
    1,
    0,
    { 0x6161b1ef, 0xea19, 0x11e1, { 0xa7, 0x78, 0x0, 0x22, 0x15, 0x64, 0x8a, 0x98 } },  /* guid */
    _T(""),
};
...
INT32 WDGPlugin::Enabled(void)
{
    ...
    {
        this->m_szColor[0] = 0;
        this->m_dgc->DisplayInputBox(this->GetPluginInfo()->lpszDiffName, _T("Input new color for guild chat text:"), this->m_szColor, _ARRAYSIZE(this->m_szColor));

        if(!this->m_szColor[0])
        {// assume default
            this->m_crChatColor = RGB(0xb4,0xff,0xb4);
...
DiffData* WDGPlugin::GeneratePatch(void)
{
    ...
    try
    {
        // MISSION: Find CGameMode::Zc_guild_chat and replace guild
        // chat color with user-provided color.

        Fd.uMask = WFD_PATTERN|WFD_WILDCARD|WFD_SECTION;
        Fd.lpData =
                  "6A 04"        // PUSH    4
                  "68 B4FFB400"  // PUSH    B4FFB4h  ; light green
                  "'?'"          // PUSH    R32
                  "6A 01"        // PUSH    1
